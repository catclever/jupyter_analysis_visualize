#!/usr/bin/env python3
"""
Generate ecommerce_analytics project with notebook and metadata
"""
...
def create_notebook():
    """Create Jupyter notebook with analysis nodes"""
    ...
    # Define all nodes - organized by analysis path
    nodes = [
        # ==================== 数据源节点 (4个) ====================
        {
            "cell_type": "code",
            ...
            "source": [
                ...
                "orders_path = os.path.join(os.path.dirname(__file__), 'orders.csv')\n",
                "load_orders_data = pd.read_csv(orders_path)\n",
                "load_orders_data['order_date'] = pd.to_datetime(load_orders_data['order_date'])\n",
                "load_orders_data\n"
            ],
            "outputs": [],
            "execution_count": None
        },
        {
            ...
            "source": [
                ...
                "customers_path = os.path.join(os.path.dirname(__file__), 'customers.csv')\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "promotions_path = os.path.join(os.path.dirname(__file__), 'promotions.csv')\n",
    ...
    notebook['cells'] = nodes
    return notebook


def create_project_metadata(notebook):
    """Create project.json metadata"""

    # Extract nodes from notebook
    nodes_info = {}
    node_depends = {}
    ...
    for cell in notebook['cells']:
        ...
        if node_id:
            nodes_info[node_id] = {
                ...
                'execution_status': 'not_executed',
                'result_format': None if node_type == 'tool' else ('parquet' if node_type != 'chart' else 'json'),
                'result_path': None
            }
            node_depends[node_id] = depends_on
    ...
    project_metadata = {
        ...
        'dag_metadata': {
            ...
            'node_types': {
                'data_source': sum(1 for n in nodes_info.values() if n['node_type'] == 'data_source'),
                'compute': sum(1 for n in nodes_info.values() if n['node_type'] == 'compute'),
                'chart': sum(1 for n in nodes_info.values() if n['node_type'] == 'chart'),
                'tool': sum(1 for n in nodes_info.values() if n['node_type'] == 'tool')
            },
            'max_depth': 0,
            'has_cycles': False
        },
        'analysis_paths': [
            {
                ...
                'description': 'Analyze daily, weekly, and category-level sales patterns',
                'nodes': ['load_orders_data', 'p1_daily_sales', 'p1_category_sales', 'p1_weekly_trend', 'p1_sales_chart']
            },
            {
                'name': 'Customer Behavior Analysis',
                'description': 'Understand customer purchase patterns, loyalty, and regional behavior',
                'nodes': ['load_orders_data', 'load_customers_data', 'p2_customer_purchase', 'p2_loyalty_analysis', 'p2_regional_behavior', 'p2_customer_chart']
            },
            {
                'name': 'Category Competition Analysis',
                'description': 'Compare category performance and identify top products',
                'nodes': ['load_orders_data', 'load_products_data', 'p3_category_comparison', 'p3_category_market_share', 'p3_top_products', 'p3_category_chart']
            },
            {
                'name': 'Marketing Effectiveness Assessment',
                'description': 'Evaluate promotion impact and payment method preferences',
                'nodes': ['load_orders_data', 'p4_promo_effectiveness', 'p4_category_promo_impact', 'p4_payment_method_analysis', 'p4_promo_chart']
            }
        ]
    }
...
if __name__ == '__main__':
    ...
    metadata = create_project_metadata(notebook)
    ...
    print(f"  - Total nodes: {metadata['dag_metadata']['total_nodes']}")
    print(f"  - Data source nodes: {metadata['dag_metadata']['node_types']['data_source']}")
    print(f"  - Compute nodes: {metadata['dag_metadata']['node_types']['compute']}")
    print(f"  - Chart nodes: {metadata['dag_metadata']['node_types']['chart']}")
    print(f"  - Tool nodes: {metadata['dag_metadata']['node_types']['tool']}")
    print(f"  - Analysis paths: {len(metadata['analysis_paths'])}")
